...
class TestMongoDB(unittest.TestCase):
    ...
    def tearDownClass(cls):
        # Close the MongoDB client once after all tests
    ...
    def test_add_log_with_ip_and_size(self):
        mongo_db.add_log("WARNING", "Suspicious download", ip="127.0.0.1", file_size=1234)
        log = mongo_db.logs_collection.find_one({"message": "Suspicious download"})
        self.assertIsNotNone(log)
        self.assertEqual(log["ip"], "127.0.0.1")
        self.assertEqual(log["file_size"], 1234)
